...
#define MEDIUM_DATASET 2000
#define LARGE_DATASET 3000
...
#define STOP_TIMER_MPI end = MPI_Wtime(); if (rank == 0) printf("%0.8f", end - start);

#define START_TIMER struct timeval start, end; gettimeofday(&start, NULL);
#define STOP_TIMER gettimeofday(&end, NULL); printf("%0.8f", time_diff(&start, &end));
